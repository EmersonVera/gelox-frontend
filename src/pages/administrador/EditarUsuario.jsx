...
const ROLES = [
  { value: 'ENCARGADO_INVENTARIO', label: 'Inventarios',  description: 'Gestión de activos y stock de bodega.', icon: <BoxIcon /> },
  { value: 'ENCARGADO_VENTAS',     label: 'Ventas',       description: 'Operaciones comerciales y clientes.',   icon: <TagIcon /> },
];
...
export default function EditarUsuario() {
  ...
  const { register, handleSubmit, setError, formState: { errors } } = useForm({
    defaultValues: { nombre: usuario.nombre ?? '', correo: usuario.correo ?? '' },
  });
  ...
  return (
    <AppLayout>
      <form onSubmit={handleSubmit(onSubmit)} noValidate className="max-w-4xl mx-auto flex flex-col gap-6 animate-fade-in-up">
        ...
        <button type="button" onClick={() => navigate('/usuarios')}
          ...
          <ArrowLeftIcon /> Todos los Usuarios
        </button>

        <div>
          <h1 className="font-display text-2xl font-extrabold text-ink tracking-tight">Gestionar Perfil de Usuario</h1>
          <p className="text-sm text-muted mt-1">
            Edite las credenciales de acceso, los permisos de rol y la configuración de seguridad del perfil de{' '}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-6">
          <div className="flex flex-col gap-5">
            ...
            <div className="bg-white rounded-2xl border border-border shadow-[0_1px_3px_rgba(0,0,0,0.07)] p-6">
              <CardHeader icon={<PersonIcon />} title="Información General" subtitle="Datos personales y de contacto" />
              ...
              <div className="flex flex-col gap-5">
                <div>
                  ...
                  <input id="nombre" type="text" placeholder="Ej. Juan Pérez García" disabled={estaDeshabilitado}
                    className={`${inputBase} ${estaDeshabilitado ? 'border-divider bg-surface text-muted cursor-not-allowed' : errors.nombre ? inputError : inputNormal}`}
                ...
                <div>
                  ...
                  <input id="correo" type="email" placeholder="usuario@gelox.com" disabled={estaDeshabilitado}
                    className={`${inputBase} ${estaDeshabilitado ? 'border-divider bg-surface text-muted cursor-not-allowed' : errors.correo ? inputError : inputNormal}`}
                    {...register('correo', { required: 'El correo es requerido.', pattern: { value: /^[^\s@]+@[^\s@]+\.[^\s@]+$/, message: 'Formato inválido.' } })} />
                  {errors.correo && <p className={errMsg}>{errors.correo.message}</p>}
                </div>

                <div>
                  <label className={lbl}>Fotografía del Perfil <span className="normal-case font-normal text-muted">(opcional)</span></label>
                  <button type="button" onClick={() => !estaDeshabilitado && fileRef.current?.click()} disabled={estaDeshabilitado}
                    className={`w-full flex flex-col items-center justify-center gap-2 py-5 border-2 border-dashed rounded-xl transition duration-300 ${estaDeshabilitado ? 'border-divider text-divider cursor-not-allowed bg-surface' : 'border-divider text-faint hover:border-primary hover:text-primary'}`}>
                    {fotoPreview ? <img src={fotoPreview} alt="Vista previa" className="w-14 h-14 rounded-full object-cover border border-border" /> : <CameraIcon />}
                    <span className="text-xs font-semibold uppercase tracking-wider">{fotoPreview ? 'Cambiar Archivo' : 'Subir Archivo'}</span>
                  </button>
                  <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={estaDeshabilitado ? undefined : handleFotoChange} />
                </div>
              </div>
            </div>

            <div className="bg-white rounded-2xl border border-border shadow-[0_1px_3px_rgba(0,0,0,0.07)] p-6">
              <CardHeader icon={<ShieldIcon />} title="Asignación de Rol" subtitle="Defina el nivel de acceso al sistema" />
              <div className="flex flex-col gap-3">
                {ROLES.map((rol) => {
                  const activo = rolSeleccionado === rol.value;
                  return (
                    <button key={rol.value} type="button" onClick={() => !estaDeshabilitado && setRolSeleccionado(rol.value)} disabled={estaDeshabilitado}
                      className={`w-full text-left flex items-start gap-4 px-4 py-4 rounded-xl border-2 transition duration-200 ${estaDeshabilitado ? 'border-border bg-surface cursor-not-allowed opacity-60' : activo ? 'border-primary bg-primary/5' : 'border-border bg-white hover:border-divider hover:bg-surface'}`}>
                      <span className={`mt-0.5 shrink-0 ${activo && !estaDeshabilitado ? 'text-primary' : 'text-faint'}`}>{rol.icon}</span>
                      <div>
                        <p className={`text-sm font-semibold ${activo && !estaDeshabilitado ? 'text-primary' : 'text-ink'}`}>{rol.label}</p>
                        <p className="text-xs text-muted mt-0.5">{rol.description}</p>
                      </div>
                      <span className={`ml-auto mt-0.5 w-4 h-4 rounded-full border-2 shrink-0 flex items-center justify-center ${activo && !estaDeshabilitado ? 'border-primary' : 'border-divider'}`}>
          ...
          <div className="flex flex-col gap-5">
            <div className="bg-white rounded-2xl border border-border shadow-[0_1px_3px_rgba(0,0,0,0.07)] p-6 flex flex-col items-center gap-3">
              <div className="w-20 h-20 rounded-full bg-primary-tint text-primary text-xl font-bold flex items-center justify-center overflow-hidden shrink-0">
                {fotoPreview ? <img src={fotoPreview} alt={usuario.nombre} className="w-full h-full object-cover" /> : getInitials(usuario.nombre)}
              </div>
            ...
            <div className={`bg-white rounded-2xl border shadow-[0_1px_3px_rgba(0,0,0,0.07)] p-6 ${estaDeshabilitado ? 'border-green-200' : 'border-[#ffb4a9]'}`}>
              <div className={`flex items-center gap-2 mb-3 ${estaDeshabilitado ? 'text-green-600' : 'text-danger'}`}>
                ...
                <span className="text-sm font-bold">{estaDeshabilitado ? 'Zona de Acción' : 'Zona de Peligro'}</span>
              </div>
              <p className="text-xs text-muted leading-relaxed mb-4">
                {estaDeshabilitado
                  ? 'Al habilitar este usuario, recuperará acceso al sistema con su rol y permisos asignados.'
                  : 'Al deshabilitar este usuario, se revocarán todos sus accesos. Podrás habilitarlo cuando lo necesites.'}
              </p>
              <button type="button" onClick={handleCambiarEstado} disabled={cambiandoEstado}
                className={`w-full flex items-center justify-center gap-2 px-4 py-2.5 text-white text-sm font-semibold rounded-xl transition duration-300 active:scale-[0.97] disabled:opacity-60 ${estaDeshabilitado ? 'bg-green-500 hover:bg-green-600' : 'bg-danger hover:bg-danger-dark'}`}>
                {cambiandoEstado ? (
                  <><span className="w-4 h-4 rounded-full border-2 border-white/40 border-t-white animate-spin" />Procesando...</>
                ) : estaDeshabilitado ? (
                  <><CheckCircleIcon />Habilitar Usuario</>
                ) : (
                  <><BanIcon />Deshabilitar Usuario</>
                )}
              </button>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-border shadow-[0_1px_3px_rgba(0,0,0,0.07)] px-6 py-4 flex items-center justify-end gap-3">
          <button type="button" onClick={() => navigate('/usuarios')} disabled={guardando}
          ...
          <button type="submit" disabled={guardando || cambiandoEstado || estaDeshabilitado}
