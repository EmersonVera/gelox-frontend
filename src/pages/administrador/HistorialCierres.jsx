// src/pages/administrador/HistorialCierres.jsx
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      className={`w-8 h-8 rounded-[8px] font-['Inter'] font-medium text-[14px] flex items-center justify-center cursor-pointer transition-colors ${
        activo ? 'bg-[#9e2016] text-white' : disabled ? 'text-[#d6d3d1] cursor-not-allowed' : 'text-[#78716c] hover:bg-[#f6f3f3]'
      }`}>
      {children}
    </button>
  );
...
export default function HistorialCierres() {
  ...
  const labelClass =
    "font-inter font-semibold text-[11px] uppercase tracking-[0.55px] text-ink mb-2 block";
  ...
  return (
    ...
    <div className="flex flex-col gap-6 animate-fade-in-up">
      <div>
        ...
        <p className="font-inter font-normal text-[16px] text-muted">
      ...
      {error && (
        <div className="bg-[#fef2f2] border border-[#fecaca] rounded-[12px] px-5 py-4 flex items-start gap-3">
          ...
          <div>
            <p className="font-['Inter'] font-semibold text-[14px] text-[#dc2626]">No se pudieron cargar los cierres</p>
            <p className="font-['Inter'] font-normal text-[13px] text-[#ef4444] mt-0.5">{error}</p>
          </div>
        </div>
      )}

      <div className="bg-white rounded-xl border border-[#f5f5f4] overflow-hidden">
        <div className="grid grid-cols-[1fr_1fr_1fr_1.2fr_40px] gap-4 px-6 py-3 border-b border-[#fafaf9]">
          {['Fecha', 'Monto Calculado', 'Monto Físico', 'Diferencia', ''].map((h) => (
            <span key={h} className="font-inter font-semibold text-[11px] uppercase tracking-[0.55px] text-[#a8a29e]">{h}</span>
          ))}
        </div>
        ...
        {cargando ? (
          <div className="flex flex-col">
            {[1, 2, 3].map((i) => (
              <div key={i} className="grid grid-cols-[1fr_1fr_1fr_1.2fr_40px] gap-4 px-6 py-4 border-b border-[#fafaf9] animate-pulse">
                {[1, 2, 3, 4, 5].map((j) => <div key={j} className="h-4 bg-[#f6f3f3] rounded" />)}
              </div>
            ))}
          </div>
        ) : cierres.length === 0 ? (
          ...
          cierres.map((c) => (
            <div key={c.id} onClick={() => navigate(`/cierres-caja/${c.id}`)}
              className="grid grid-cols-[1fr_1fr_1fr_1.2fr_40px] gap-4 px-6 py-4 border-b border-[#fafaf9] hover:bg-surface/50 cursor-pointer transition-colors items-center">
              <span className="font-inter font-medium text-[15px] text-ink">{c.fecha}</span>
              <span className="font-inter font-medium text-[15px] text-ink">{formatCOP(c.montoCalculadoTotal)}</span>
              <span className="font-inter font-medium text-[15px] text-ink">{formatCOP(c.montoFisicoTotal)}</span>
              <DiferenciaBadge valor={c.diferenciaTotal} />
              <svg width="16" height="16" fill="none" viewBox="0 0 16 16" className="text-[#a8a29e] justify-self-end">
        ...
        <div className="flex items-center justify-between px-6 py-4">
          <p className="font-inter font-normal text-[14px] text-muted">
            Mostrando {cierres.length} de {total} cierres registrados
          </p>
          <div className="flex gap-1 items-center">
            <PaginaBtn onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}>‹</PaginaBtn>
            {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => i + 1).map((n) => (
              <PaginaBtn key={n} activo={n === page} onClick={() => setPage(n)}>{n}</PaginaBtn>
            ))}
            <PaginaBtn onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages}>›</PaginaBtn>
          </div>
        </div>
      </div>
    </div>
